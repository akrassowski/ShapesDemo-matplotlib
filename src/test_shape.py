...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        self.matplotlib = MagicMock()  # only mock the values that matter
        #matplotlib.args.graph_xy = (240, 270)
        #matplotlib.args.figure_xy = (2, 3)
        self.matplotlib.axes.get_xlim.return_value = (0, 240)
        self.matplotlib.axes.get_ylim.return_value = (0, 270)

        self.circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="GREEN",
            xy=(33, 33), size=33,
            angle=45, fill=0
        )
        self.square = Shape(
            matplotlib=self.matplotlib, seq=72, which="S",
            color="RED",
            xy=(30, 30), size=30,
        )
        self.triangle = Shape(
            matplotlib=self.matplotlib, seq=73, which="T",
            color="YELLOW",
            xy=(30, 30), size=30,
        )
    ...
    def test_example_colors(self):
        fcolor, ecolor = self.circle.face_and_edge_color_code()
        self.assertEqual(fcolor, COLOR_MAP['GREEN'])
        self.assertEqual(ecolor, COLOR_MAP['BLUE'])
        fcolor, ecolor = self.square.face_and_edge_color_code()
    ...
    def test_color_fill0_blue(self):
        circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="BLUE", size=30, xy=(33, 33), fill=0
        )
        fcolor, ecolor = circle.face_and_edge_color_code()
    ...
    def test_color_fill1_blue(self):
        circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="BLUE", size=30, xy=(33, 33), fill=1
        )
        fcolor, ecolor = circle.face_and_edge_color_code()
    ...
    def test_color_fill2_blue(self):
        circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="BLUE", size=30, xy=(33, 33), fill=2
        )
        fcolor, ecolor = circle.face_and_edge_color_code()
        self.assertEqual(fcolor, COLOR_MAP['WHITE'])
        self.assertEqual(ecolor, COLOR_MAP['BLUE'])
    ...
    def test_color_compute_green(self):
        circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="GREEN", size=30, xy=(33, 33), fill=0
        )
        fcolor, ecolor = circle.face_and_edge_color_code()
        self.assertEqual(fcolor, COLOR_MAP['GREEN'])
        self.assertEqual(ecolor, COLOR_MAP['BLUE'])

    def test_color_compute_pub(self):
        circle = Shape(
            matplotlib=self.matplotlib, seq=71, which="C",
            color="GREEN", size=30, xy=(33, 33), fill=0, pub=True
        )
        fcolor, ecolor = circle.face_and_edge_color_code()
        self.assertEqual(fcolor, COLOR_MAP['GREEN'])
        self.assertEqual(ecolor, COLOR_MAP['BLACK'])
